...
Sonic.prototype = { 
   ...
   atualizar: function() { 
      ...
      if (this.teclado.pressionada(SETA_DIREITA)) {
         ...
         this.x -= this.velocidade;  // E aqui é sinal de menos!
            ...
            if (! this.andando || this.direcao != SONIC_PULANDO) {
                this.sheet.linha = 0; // linha de pulo
                this.sheet.coluna = 0; 
            }
            this.andando = true;
            this.direcao = SONIC_PULANDO;
            //this.sheet.proximoQuadro();
            this.y -= this.velocidade; // aqui é sinal de menos para pular
      }

      else if (this.teclado.pressionada(SETA_BAIXO)) {
            if (! this.andando || this.direcao != SONIC_DESCENDO) {
                this.sheet.linha = 0; // linha de descendo
                this.sheet.coluna =0;
            }
            this.andando = true;
            this.direcao = SONIC_DESCENDO;
            //this.sheet.proximoQuadro();
            this.y += this.velocidade; // sinal de mais para descer
      }
   ...
   desenhar: function() { 
    ...
    /*atirar: function() {
      ...
      atirar: function() {
      var tiro = new Tiro(this.context, imgExplosao);
      tiro.x = this.x + 5;
      tiro.y = this.y - 10;
